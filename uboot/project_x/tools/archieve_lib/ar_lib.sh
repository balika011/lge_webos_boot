...
if [ "`echo "$1" | grep -c -w ccache`" != "0" ]; then
AR_TOOL="$1 $2"
AR_TOOL_FLAG=$3
AR_NAME=$4
AR_FILE=$5
...
MYCMD="$AR_TOOL $AR_TOOL_FLAG "
index=0
MY_LIMIT=300
MY_INPUT=""
for i in `cat $AR_FILE`
do
	if [ ! -e $i ]; then
		echo "$i not existed,$AR_NAME Archieve Error"
		rm -rf $AR_NAME
		exit 1;
	fi
	...
	index=`expr $index + 1`
	MY_INPUT="$i $MY_INPUT"
	if [ $index -gt $MY_LIMIT ]; then
		$MYCMD $AR_NAME $MY_INPUT
		MY_INPUT=""
		index=""
	fi
done
...
if [ "$MY_INPUT" != "" ]; then
	#echo "$MYCMD $AR_NAME $MY_INPUT"
	$MYCMD $AR_NAME $MY_INPUT
fi
#echo "$AR_NAME Archieve Completed"
