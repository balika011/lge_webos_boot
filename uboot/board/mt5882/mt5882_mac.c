...
int SetHWMac( void )
{
	char * macaddr;
	u8  macAddr[6] = {0};
	macaddr = getenv("ethaddr");

	if( macaddr != NULL )
	{
		macAddr[0]	 = (u8)simple_strtoul(&(macaddr[0]),  NULL, 16);
		macAddr[1]	 = (u8)simple_strtoul(&(macaddr[3]),  NULL, 16);
		macAddr[2]	 = (u8)simple_strtoul(&(macaddr[6]),  NULL, 16);
		macAddr[3]	 = (u8)simple_strtoul(&(macaddr[9]),  NULL, 16);
		macAddr[4]	 = (u8)simple_strtoul(&(macaddr[12]), NULL, 16);
		macAddr[5]	 = (u8)simple_strtoul(&(macaddr[15]), NULL, 16);
		printf("SetHWMac : macaddr 0x%x:0x%x:0x%x:0x%x:0x%x:0x%x \n",macAddr[0],macAddr[1],macAddr[2],macAddr[3],macAddr[4],macAddr[5]);
	}
  /* Set Mac Address */
  vWriteMAC(RW_MY_MAC_H, (macAddr[0]<<8) | (macAddr[1]<<0));
  vWriteMAC(RW_MY_MAC_L, (macAddr[2]<<24) | (macAddr[3]<<16) | (macAddr[4]<<8) | (macAddr[5]<<0));
}
...
void hwcfgsetup(void)
{
	...
	printf ("hwcfgsetup 0xd164=(0x%x)\n", IO_READ32(CKGEN_BASE, 0x164));
    vDmaInit();  //shengming
    vMacInit();
...
//    dev->name[0] = 'M';
	memcpy(dev->name, "mt5882mac", strlen("mt5882mac") );
	dev->name[9] = '\0';
    ...
    dev->write_hwaddr = mmac_write_hwaddr;
...
static int mmac_write_hwaddr(struct eth_device *dev)  
{
	//firstly set this register.
	IO_WRITE32(CKGEN_BASE,0x164, IO_READ32(CKGEN_BASE, 0x164) & (~(0x1 << 1)));

	/* Set Mac Address */
	vWriteMAC(RW_MY_MAC_H, (dev->enetaddr[0]<<8) | (dev->enetaddr[1]<<0));
	vWriteMAC(RW_MY_MAC_L, (dev->enetaddr[2]<<24) | (dev->enetaddr[3]<<16) | (dev->enetaddr[4]<<8) | (dev->enetaddr[5]<<0));
	return 0;
}
