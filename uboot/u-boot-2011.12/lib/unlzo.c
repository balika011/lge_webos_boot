...
inline unsigned char Rd8Data(unsigned char * s)
{
    return s[0];
}

inline unsigned int Rd32Data(unsigned char * s)
{
    return ( s[0]<<24 | s[1]<<16 | s[2]<<8 | s[3] );
}

unsigned int unlzo_get_decompsize(unsigned char * buf)
{
	if (memcmp(buf, lzop_magic, sizeof(lzop_magic)) != 0)
    {
        printf("it's not lzo header\n");
        return 0;
    }

	return Rd32Data(buf + sizeof(lzop_magic));
}

int unlzo_get_hdroffset(u8 *input, int in_len)
{
	int offset = 0;
	
	while (offset < in_len)
	{
		if (memcmp(input+offset, lzop_magic, sizeof(lzop_magic)) == 0)
			return offset;
		
		/* assume lzop hdr is aligned by sizeof(lzop_magic) */
