...
int do_bootm(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	if( gSecureFlag != (SECURE_FLG_KERNEL|SECURE_FLG_LGAPP|SECURE_FLG_OTHERS) )
	{
		printf("SECURE BOOT in bootm:Kernel/lgapp/lginit should be verify before bootm Seucre flag = 0x%x \n ",gSecureFlag);
#ifndef WEBOS_DEVELOPMENT
#else
		return 1;
#endif
	}	
	printf("SECURE BOOT in bootm:Secure flag = 0x%x \n ",gSecureFlag);
		...
		ulong *os_len)
{
	...
	*os_data = *os_len = 0;
	...
	case IMAGE_FORMAT_LEGACY:
		printf ("[%4d] ## Booting kernel from Legacy Image at %08lx ...\n", readMsTicks(), 
				img_addr);
...
int do_usb_upgrade(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
    ...
    char part_name[16];
    u8 part_num = 0;
    u32 part_size = 0, part_offset = 0;
    char r_part_name[16];
    u8 r_part_num = 0;
    u32 r_part_size = 0, r_part_offset = 0;
...
#endif // CONFIG_NAND_BOOT && CONFIG_CMD_JFFS2 && CONFIG_JFFS2_CMDLINE
#else
#if (0)//defined(CONFIG_NAND_BOOT) || defined(CC_EMMC_BOOT)) && defined(CONFIG_CMD_JFFS2) && defined(CONFIG_JFFS2_CMDLINE)
int do_mtd_update(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
    char *local_args[7];
    char p0[64], p1[32], p2[32], p3[32];
