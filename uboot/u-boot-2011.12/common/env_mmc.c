...
#if	defined(CMD_SAVEENV) && !defined(CFG_LG_CHG)
...
void env_relocate_spec(void)
{
	...
	{
		printf("Sread Environment to %s...\n", env_name_spec);
		printf("[%s] env_part: %s\n", __func__, env_partinfo->name);
		printf("read to %x (len=%d,ptr = 0x%p) \n", (ulong)env_partinfo->offset, (ulong)env_partinfo->size, env_ptr);

		if (read_env(mmc, (ulong)env_partinfo->size, (ulong)env_partinfo-> offset, (u_char *)env_ptr)) 
			return set_default_env(NULL);
		if (read_env(mmc, (ulong)env_partinfo->size, (ulong)env_partinfo-> offset, buf)) 
			return set_default_env(NULL);
		env_import(buf, 1);
	}
	else
