...
static void SIF_MtkCfgPinSetFunc(UINT8 u1Channel)
{
    ...
    else if (_SIF_ISR_IsPdwncMasterEnable() && u1Channel == 3) // PDWNC SIF0 : OPCTRL4/OPCTRL5 use PD SIF1
    {
#ifdef CC_LGE_PROTO_PCBA
        u4Reg = IO_READ32(PDWNC_BASE, 0xB8);
        // PDMUX1[2:0] != 3
        if ((u4Reg & 0x00000007) == 0x00000003)
        {
            u4Reg &= ~(0x00000003);
        }
        // PDMUX1[5:3] != 3
        if ((u4Reg & 0x00000038) == 0x00000018)
        {
            u4Reg &= ~(0x00000018);
        }
    ...
    else if (_SIF_ISR_IsPdwncMaster1Enable() && u1Channel == 4) // PDWNC SIF1: OPCTRL11/OPCTRL12 use PD SIF1
    {
#ifdef CC_LGE_PROTO_PCBA
        u4Reg = IO_READ32(PDWNC_BASE, 0xB4);
        // PDMUX0[13:11] != 1
