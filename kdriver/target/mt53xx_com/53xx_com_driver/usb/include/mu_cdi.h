...
typedef struct _MUSB_ControlIrp
{
    ...
    void* pCompleteParam;
    //for wifi f/w download
    const uint8_t* pOutDataBuffer;
    uint32_t dwOutDataLength;
    int32_t dwRemainOutDataLength;
    uint8_t bControlTransferStage;
...
typedef struct _MUSB_Irp
{
    ...
    uint8_t bAllowDma;
    uint8_t bIsTx;
    void *pEnd;  //MGC_EndpointResource *
...
#ifdef CC_MT8563
/**
 * power control the bus.
 * @param hBus the bus "handle"
 */
extern void MUSB_PdwncBus(MUSB_BusHandle hBus, uint32_t powerState);
#endif
...
/**
 * MUSB_EyePatternFineTune.
 * This is specifically intended for fine-tuning of eye pattern of USB PHY.
 * The USB PHY setting of signal quality maybe different for each IC 
 * type/version. 
 * This function maybe empty, if there is not necessary to fine-tune the eye 
 * pattern.
 *
 */
#if defined(CONFIG_ARCH_MT85XX)
void MUSB_EyePatternFineTune(void);
#endif
/** Enable high speed 
