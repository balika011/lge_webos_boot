...
static const char MGC_aDescriptorData[] = 
{
    ...
    2+18,			/* Product ID: 8530 Disk */
    MUSB_DT_STRING,
    '8', 0, '5', 0, '3', 0, '0', 0, ' ', 0, 'D', 0, 'i', 0, 's', 0, 'k', 0,

    2+24,			/* Serial #: SA5 MTK1358  */
    MUSB_DT_STRING,
    'S', 0, 'A', 0, '5', 0, ' ', 0,
    'M', 0, 'T', 0, 'K', 0, '1', 0,
    '3', 0, '5', 0, '8', 0, ' ', 0,
...
static MGC_MsdStandardInquiryData *MGC_MsdRbcInquiryData = NULL;
static const MGC_MsdStandardInquiryData MGC_MsdRbcInquiryData_x =
{
...
/** Current sense data */
static MGC_MsdScsiSenseData* MGC_MsdSenseData = NULL;
//static MGC_MsdScsiSenseData MGC_MsdSenseData_x;

/** Capacity response */
static MGC_MsdCapacityResponse* MGC_MsdCapacity = NULL ;
static MGC_MsdFormatCapacityResponse* MGC_MsdFormatCapacity = NULL;
...
#if 1
#define USB_MSDFN_DISK_SIZE         512*1024     //16*1024*1024
STATIC uint8_t* MGC_aMsdData =  NULL; //(uint8_t*)0x024fc000;
static uint32_t MGC_dwMsdBlockCount = USB_MSDFN_DISK_SIZE / 512; //(0x05b00000) / 512;
#else
...
BOOL MGC_SetMsdDiskBuf(void *pBufAddr)
{
    if(!pBufAddr)
    {
        return FALSE;
    ...
    LOG(0, "[USB MSDFN] MGC_aMsdData = 0x%08X\n", (uint32_t)pBufAddr);
    MGC_aMsdData = (uint8_t *) pBufAddr;
    return TRUE;
}
			       ...
			       MUSB_State State)
{
    if(!MGC_aMsdData)
    { 
        uint32_t u4MemSize = USB_MSDFN_DISK_SIZE;
        void * pvBufPtr = MUSB_MemAlloc(u4MemSize);
        //unsigned long order = get_order(u4MemSize);
        //void * pvBufPtr = (void *) __get_free_pages(GFP_KERNEL, order);
        
        LOG(0, "Alloc %dKB Memory for MGC_aMsdData@0x%08X.\n", u4MemSize/1024, pvBufPtr);
        MGC_SetMsdDiskBuf(pvBufPtr);

        ASSERT(MGC_aMsdData);
    }

	if(!MGC_MsdRbcInquiryData)
	{
	    MGC_MsdRbcInquiryData = (MGC_MsdStandardInquiryData*)MUSB_MemAlloc(sizeof(MGC_MsdStandardInquiryData));
        ASSERT(MGC_MsdRbcInquiryData);
		LOG(0, "Alloc %d Memory for MGC_MsdRbcInquiryData.\n", sizeof(MGC_MsdStandardInquiryData));
		MUSB_MemCopy((void*)MGC_MsdRbcInquiryData,(void*)&MGC_MsdRbcInquiryData_x,sizeof(MGC_MsdStandardInquiryData));
	}

	if(!MGC_MsdSenseData)
	{
	    MGC_MsdSenseData = (MGC_MsdScsiSenseData*)MUSB_MemAlloc(sizeof(MGC_MsdScsiSenseData));
        ASSERT(MGC_MsdSenseData);
		LOG(0, "Alloc %d Memory for MGC_MsdScsiSenseData.\n", sizeof(MGC_MsdScsiSenseData));
		MUSB_MemSet((void*)MGC_MsdSenseData,0,sizeof(MGC_MsdScsiSenseData));
	}    
	
	if(!MGC_MsdCapacity)
	{
	    MGC_MsdCapacity = (MGC_MsdCapacityResponse*)MUSB_MemAlloc(sizeof(MGC_MsdCapacityResponse));
        ASSERT(MGC_MsdCapacity);
		LOG(0, "Alloc %d Memory for MGC_MsdCapacity.\n", sizeof(MGC_MsdCapacityResponse));
		MUSB_MemSet((void*)MGC_MsdCapacity,0,sizeof(MGC_MsdCapacityResponse));		
	}  

	if(!MGC_MsdFormatCapacity)
	{
	    MGC_MsdFormatCapacity = (MGC_MsdFormatCapacityResponse*)MUSB_MemAlloc(sizeof(MGC_MsdFormatCapacityResponse));
        ASSERT(MGC_MsdFormatCapacity);
		LOG(0, "Alloc %d Memory for MGC_MsdFormatCapacity.\n", sizeof(MGC_MsdFormatCapacityResponse));
		MUSB_MemSet((void*)MGC_MsdFormatCapacity,0,sizeof(MGC_MsdFormatCapacityResponse));		
	}  	

	if(!MGC_aJunk)
	{
		MGC_aJunk = (uint8_t*)MUSB_MemAlloc(512);
		ASSERT(MGC_aJunk);
		LOG(0, "Alloc %d Memory for MGC_aJunk.\n", 512);
		MUSB_MemSet((void*)MGC_aJunk,0,512);		
	} 
...
static void MGC_MsdCommand(const MGC_MsdCbw* pCbw)
{
    ...
    uint8_t bOpcode = pCbw->aCbwCb[0];
  	...
  	if(!MGC_aMsdData)
  	{
	    LOG(0, "Ram disk space is NULL");
	  }

  	MUSB_ASSERT(MGC_aMsdData);
	...
	MGC_MsdTxDataIrp.dwLength = sizeof(MGC_MsdCapacityResponse);
	...
	MGC_MsdTxDataIrp.dwLength = sizeof(MGC_MsdFormatCapacityResponse);
	pCsw->dCswDataResidue = dwLength ? (dwLength - MGC_MsdTxDataIrp.dwLength) : 0;
	MGC_MsdSenseData->bResponseCode = 0x70;
	bStartTx = TRUE;
	bStatus = 0;
