...
static uint32_t MGC_SicdConfigureDevice(MUSB_SicdDevice *pSicdDevice)
{
    ...
    pControlIrp->bIsrCallback      = FALSE;
...
static void MGC_SicdDeviceInfo(MUSB_SicdProtocol *pSicdProtocol)
{
    ...
    if(!pSicdProtocol->bIsGetPartialObj)
    {
        LOG(0, "SICD: Not Support GetPartialObject.\n");
    }
        ...
        if (pIface && (pIface->bNumEndpoints == 3))
        {
            ...
            else
            {
                ...
                if ( pSicdDevice )
                {
                     ...
                     * Not support MTP device of APPLE.
                     * e.g: iPod touch or iPhone. 
                     */
                    pDevice = pSicdDevice->pUsbDevice;
                    if (pDevice->DeviceDescriptor.idVendor == 0x05AC)
                    {
                        LOG(5, "Not support iPhone/iPod touch.\n");
                        return FALSE;
                    }

                    LOG(5, "MTP/PTP Get MS OS Descriptor\n");
