 ...
 *****************************************************************************/
#if defined(CONFIG_ARCH_MT85XX)
extern uint8_t fgUsbResetPortPower(MUSB_Device *pUsbDevice);
#endif
...
#if defined (CONFIG_ARCH_MT85XX)
void MGC_HubPortPowerOnOffCallback(void *pContext, MUSB_ControlIrp *pControlIrp)
{
    MUSB_Hub *pHubDevice;
    int32_t  i4LockStatus = 0;
       
    pHubDevice = (MUSB_Hub *)pContext;
    i4LockStatus = x_sema_unlock(pHubDevice->ctrlLock);
    UNUSED(i4LockStatus);
}

void MUSB_HubPortPowerOff(MUSB_Hub *pHubDevice, uint32_t u4PortNum)
{
    uint32_t u4Status = 0xffffffff;
    int32_t  i4LockStatus = 0;

    u4Status = MUSB_HubClearPortFeature(pHubDevice,  (uint16_t)MUSB_HUB_PORT_POWER_FEATURE, 
                             MGC_HubPortPowerOnOffCallback, u4PortNum);

    if(MUSB_SUCCESS == u4Status)
    {
        i4LockStatus = x_sema_lock(pHubDevice->ctrlLock, X_SEMA_OPTION_WAIT);
    }

    LOG(0, "Hub Port#%d Off\n", u4PortNum);
    
    UNUSED(u4Status);    
    UNUSED(i4LockStatus);
}

void MUSB_HubPortPowerOn(MUSB_Hub *pHubDevice, uint32_t u4PortNum)
{
    uint32_t u4Status = 0xffffffff;
    int32_t  i4LockStatus = 0;

    u4Status = MUSB_HubSetPortFeature(pHubDevice,  (uint16_t)MUSB_HUB_PORT_POWER_FEATURE, 
                             MGC_HubPortPowerOnOffCallback, u4PortNum);
...
BOOL fgUsbHubPortPower(MUSB_Hub *pHubDevice, uint8_t bPortNum, uint8_t fgEnable)
{
    VERIFY(OSR_OK == x_sema_create(&pHubDevice->ctrlLock, X_SEMA_TYPE_BINARY,
                                   X_SEMA_STATE_LOCK));

    if(fgEnable)
    {
        MUSB_HubPortPowerOn(pHubDevice, bPortNum);
        LOG(0, "#%d HubPortPowerOff\n", bPortNum);    
    }
    else
    {
        MUSB_HubPortPowerOff(pHubDevice, bPortNum);
        LOG(0, "#%d HubPortPowerOff\n", bPortNum);    
    }

    VERIFY(OSR_OK == x_sema_delete(pHubDevice->ctrlLock));
    
    return TRUE;
}

uint8_t fgUsbResetPortPower(MUSB_Device *pUsbDevice)
{
    MUSB_Device *pParentDevice = NULL;
    MUSB_Hub *pHubDevice = NULL;
    uint8_t bPortNum = 0xff;
    ...
    pParentDevice = pUsbDevice->pParentUsbDevice;

    //Is device attached on hub port?
    if(pParentDevice)
    {
        if(pParentDevice->DeviceDescriptor.bDeviceClass == MUSB_CLASS_HUB)
        {
            MGC_HubFindHubDevice(pUsbDevice, &pHubDevice);
        }
    }

    if(pHubDevice)
    {
        //Device is attached on hub port
        //Power off the hub port
        bPortNum = pUsbDevice->bHubPort;        
        bStatus = fgUsbHubPortPower(pHubDevice, bPortNum, FALSE);
        MUSB_Sleep(200);
        bStatus = fgUsbHubPortPower(pHubDevice, bPortNum, TRUE);
    }
    else
    {
        //device on root port
        bPortNum = pUsbDevice->pPort->bUsbPort;
        bStatus = fgUsbVbusEnable(bPortNum, FALSE);
        MUSB_Sleep(200);
        bStatus = fgUsbVbusEnable(bPortNum, TRUE);
    }

    return bStatus;
}

void MUSB_RootHubPortsPowerOff(void)
{
    MUSB_Port *pUsbPort = NULL;
    MGC_Port *pPort = NULL;
    uint32_t i = 0, j = 0;
    int32_t  i4LockStatus = 0;
    MUSB_HubDriverContext *phubDriverContext = NULL;
    MUSB_DeviceDriver *pHubDriver = MGC_GetHubClassDriver();
    MUSB_Hub *pHubDevice = NULL;

    if(!MUSB_fgIsInit()) return;

    if(!pHubDriver) return;

    phubDriverContext = (MUSB_HubDriverContext *)pHubDriver->pPrivateData;
        ...
        if (pUsbPort)
        {
            ...
            if (pPort)
            {
                if(pPort->pRootDevice && 
                   pPort->pRootDevice->DeviceDescriptor.bDeviceClass == 0x9 /* HUB CLASS CODE*/)
                {                
                    if(MUSB_SUCCESS == MGC_HubFindHubDevice(pPort->pRootDevice, &pHubDevice))
                    {
                        if(pHubDevice && 
                            pHubDevice->bState >= MUSB_HUB_STATE_POWER_ON_COMPLETE &&
                            phubDriverContext->bNumOfHubActive)
                        {
                            MGC_DeEnumerateHub(pHubDevice);
                            ...
                            for(j=0; j<pHubDevice->hubDscr.bNbrPorts && //pPort->pRootDevice->bPortCount
                                     j<MUSB_HUB_MAX_PORTS; j++)
                            {
                                MUSB_HubPortPowerOff(pHubDevice, j+1);
                            }
