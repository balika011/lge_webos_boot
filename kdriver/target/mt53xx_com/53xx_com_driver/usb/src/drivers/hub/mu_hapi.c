...
MUSB_DeviceDriver *MGC_GetHubClassDriver(void)
{
...
static uint8_t MGC_HubConnect(void *pDriverPrivateData, MUSB_BusHandle busHandle, MUSB_Device *pUsbDevice, const uint8_t *pPeriList)
{
    ...
    if( pUsbDevice->DeviceDescriptor.idVendor == 0x03EB && 
        pUsbDevice->DeviceDescriptor.idProduct == 0x0902 )
    {
        LOG(1, "This HUB is not support\n");
        return FALSE;
    }
