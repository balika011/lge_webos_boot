...
typedef struct
{
    ...
    uint8_t bError;
    uint8_t bRemovable;
    uint8_t aModeSenseData[0xC0]; // aModeSenseData will use dma mode, it is start & end address must 32bytes align to avoid overwrite others, due to cache line & dma issue.
    uint8_t bLun;
    uint8_t bLunCount;
    uint8_t bNoMedium;
