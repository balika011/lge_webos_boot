...
#define USB_FS_DELAY_TIME_MS    (10)
...
static uint8_t MGC_BotProtocolStartDevice(void *pProtocolData, const MUSB_Device *pDevice)
{
    ...
    pBot->ControlIrp.bIsrCallback = FALSE;
...
static uint32_t MGC_BotProtocolRWRetry(MGC_MsdBotIrp *pBotIrp)
{
    ...
    {
        (void)MUSB_Sleep(USB_FS_DELAY_TIME_MS);
    }
...
static void MGC_BotCbwComplete(void *pParam, const MUSB_Irp *pIrp)
{
    ...
    {
        //2009.02.07
        if( 0x0471 == pBot->pDevice->DeviceDescriptor.idVendor &&
            0x016A == pBot->pDevice->DeviceDescriptor.idProduct)
        {
            (void)MUSB_Sleep(10); //10ms
        }
