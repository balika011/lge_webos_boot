...
#ifdef CUST_LCCT_DRV
extern UINT8 ui1_karaoke_status;
#endif
    ...
    MUSB_Device *pUsbDevice, const uint8_t *pPeripheralList)
{
    ...
    if (!pHidDevice)
    {
        MUSB_ClosePipe(intrInPipe);//klock
        return FALSE;
    }    
    ...
    MUSB_BusHandle hBus, MUSB_Device *pUsbDevice)
{
	...
	//Check sony karaok device and  clean the flag 
	if((pUsbDevice->DeviceDescriptor.idProduct ==0x05a4)
	&&(pUsbDevice->DeviceDescriptor.idVendor==0x054c))
	{
		#ifdef CUST_LCCT_DRV
		LOG(0,"Karaok device Disconnect !! \n");
		ui1_karaoke_status = 0;
		#endif
	}
