                               ...
                               uint16_t wPacketSize, uint8_t bContinue, void *pCurrentIrp)
{
    ...
    case MUSB_ENDPOINT_XFER_INT:
...
#ifdef USB_WIFI_DEBUG
        {
            ...
            if (_pUsbBulkInPipe == pIrp->hPipe)
            {   
                _fgUsbIsBulkInPipe = TRUE;
            }
            else
            {
                _fgUsbIsBulkInPipe = FALSE;
            }
        }
#endif
...
#ifdef USB_WIFI_DEBUG          
        extern MUSB_Pipe _pUsbBulkInPipe;
        extern uint8_t *_pUsbRxBufLast;

        if (_pUsbBulkInPipe == pIrp->hPipe)
        {
            if(*(uint32_t*)_pUsbRxBufLast != 0)
            {
                LOG(5, "STOP Here!\n");
            }
        ...
        case MUSB_ENDPOINT_XFER_INT:
            ...
            if (pEnd->dwTxOffset >= dwReqLength)
            {
                ...
                if (pIrp->bAllowShortTransfer && (0 == (pEnd->dwTxOffset % pEnd->wPacketSize)))
                {
                    ...
                    {
                        pBuffer = pIrp->pBuffer; //avoid pBuffer is NULL
                        ...
                        uint8_t bIsTx, void *pCurrentIrp)
{
    ...
    case MUSB_ENDPOINT_XFER_CONTROL:
        ...
        if(MUSB_STATUS_STAGE == pControlIrp->bControlTransferStage)
        {
            if (pControlIrp && pControlIrp->pfIrpComplete && pControlIrp->bIsrCallback)
            {
