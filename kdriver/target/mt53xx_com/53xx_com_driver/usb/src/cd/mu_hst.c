...
#ifdef USB_VBUS_CONT_P1_P2 //for Sonic.
#include "logsave.h"
#include "x_dt.h"
#endif
...
//  Fix GENERIC USB Storage-SDC I19B, it do not return any data in pDeviceDesc->iProduct string.
    ...
    case MGC_EnumStateGetFullDevice:    
		...
		if(NULL == pEnumData->pDevice->pStrManufacturerBuffer )
		{
			LOG(0, "pStrManufacturerBuffer allocate fail .\n");
			return;
		}
            MUSB_MemCopy(pEnumData->aSetupTx, MGC_aGetDeviceDescriptor, sizeof(MGC_aGetDeviceDescriptor));
            pRequest = (MUSB_DeviceRequest *)pEnumData->aSetupTx;            
		...
		if(NULL ==pEnumData->pDevice->pStrProductBuffer )
		{
			LOG(0, "pStrProductBuffer allocate fail .\n");
			return;
		}
        ...
        if ((pDeviceDesc->iSerialNumber != 0) && (!pEnumData->pDevice->pStrSerialNumBuffer))
        {
            pEnumData->pDevice->pStrSerialNumBuffer =
                (uint8_t *)MUSB_MemAlloc(MGC_MAX_STRING_LENGTH);       
		if(NULL == pEnumData->pDevice->pStrSerialNumBuffer )
		{
			  LOG(0, "pStrSerialNumBuffer allocate fail .\n");
			  return;
		}
...
#else
            ///When configuration is bigger than 1, the pointer is wrong
            pEnumData->pDevice->apConfigDescriptors[pEnumData->bIndex] = (MUSB_ConfigurationDescriptor *)pEnumData->Irp.pInBuffer;
#endif
        }
        if (++pEnumData->bIndex>=pEnumData->bCount)
        {
...
#if !defined (CONFIG_ARCH_MT85XX)
            #ifdef MUSB_LOADER
            if(pEnumData->pDevice->pParentUsbDevice) // device on a hub
            {
                HAL_TIME_T timeStamp;
                HAL_GetTime(&timeStamp);
                Printf("u4timeStamp=%u:%u, %d , F:%s\n", timeStamp.u4Seconds,timeStamp.u4Micros,__LINE__,__FILE__);
                if(pEnumData->pDevice->bDeviceClass == MUSB_CLASS_MASS_STORAGE)
                {
                    MUSB_SetMassOnHubFlag(1);
                }
            }
            #endif
#endif
                            ...
                            uint8_t bSpeed, MUSB_pfHubEnumerationComplete pfHubEnumerationComplete)
{
    ...
    if (pDevice)
    {
        ...
        pEnumData->Irp.bIsrCallback = FALSE;
...
#if defined (CONFIG_ARCH_MT85XX)
    //wifi porting
    pIrp->bControlTransferStage = MUSB_SETUP_STAGE;
    pIrp->dwRemainOutDataLength = pIrp->dwOutDataLength;
#endif
...
{    
    ...
    pHubDevice->ctrlIrp.bIsrCallback = FALSE;
