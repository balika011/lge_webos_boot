...
#if 0
	#ifdef MUSB_HDRC
	Printf("[USB]Define MUSB_HDRC.\n");
	#endif
	#ifdef MUSB_MHDRC
	Printf("[USB]Define MUSB_MHDRC.\n");
	#endif
	#ifdef MGC_OTG
	Printf("[USB]Define MGC_OTG.\n");
	#endif
	#ifdef MGC_HOST
	Printf("[USB]Define MGC_HOST.\n");
	#endif
	#ifdef MUSB_HSHC
	Printf("[USB]Define MUSB_HSHC.\n");
	#endif
	#ifdef MUSB_OTG
	Printf("[USB]Define MUSB_OTG.\n");
	#endif
	#ifdef MUSB_HOST
	Printf("[USB]Define MUSB_HOST.\n");
	#endif
	#ifdef MGC_FUNCTION
	Printf("[USB]Define MGC_FUNCTION.\n");
	#endif
	#ifdef MUSB_FUNCTION
	Printf("[USB]Define MUSB_FUNCTION.\n");
	#endif
#endif
...
#endif
		#ifdef CC_MT8563
		pWrapper->PortImpl.pfPdwncPort = MGC_HdrcPdwncPort;
		#endif
...
#ifdef CC_MT8563
void MUSB_PdwncBus(MUSB_BusHandle hBus, uint32_t powerState)
{
	MGC_Port *pPort = (MGC_Port *)hBus;

	pPort->pfPdwncPort(pPort, powerState);
}
#endif
/*
 * Implementation
        ...
        for (bEnd = 1; bEnd < pPort->bEndCount; bEnd++)
        {
            ...
            if (bIsTx)
            {
                if ((!pEnd->bIsClaimed) && (wPacketSize <= pEnd->wMaxPacketSizeTx))
                {
                    ...
                    LOG(5, "Bind LEP#%02X to BEP#%02X\n", pEnd->bLocalEnd, pEnd->bBusEnd);
