...
#ifdef CUST_SOEM_DRV
UINT8 ui1_karaoke_status = 0; //0: no device 1: yes
#endif
    ...
    MUSB_Device *pChildUsbDevice, uint32_t dwChildUsbDeviceStatus)
{
    ...
    {
              //Check sony karaok device and  set the flag 
		if((pChildUsbDevice->DeviceDescriptor.idProduct ==0x05a4)
					&&(pChildUsbDevice->DeviceDescriptor.idVendor==0x054c))
			{
						prSocketInfo->u4IsKaraokeDevice=1;	
						#ifdef CUST_SOEM_DRV
						LOG(0,"Karaok device Connect !! \n");
						ui1_karaoke_status = 1;
						#endif
        		}
