...
static uint8_t MGC_UcosEp0Lock(void *pPrivate, uint16_t wIndex)
{
    MGC_UcosSystem *pSystem;
    ...
    i4_ret = x_sema_lock(pSystem->hEp0Lock, X_SEMA_OPTION_WAIT);
    MUSB_ASSERT(OSR_OK == i4_ret);

    LOG(5, "EP0 Lock  @0x%08X\n", pSystem->hEp0Lock);
...
static uint8_t MGC_UcosEp0Unlock(void *pPrivate, uint16_t wIndex)
{
    MGC_UcosSystem *pSystem;
    ...
    /* Release the semaphore */
    i4_ret = x_sema_unlock(pSystem->hEp0Lock);
    MUSB_ASSERT(OSR_OK == i4_ret);
    UNUSED(i4_ret);

    LOG(5, "EP0 UnLock@0x%08X\n", pSystem->hEp0Lock);
...
static void MGC_UcosDestroyController(MGC_UcosSystem *pSystem)
{
    ...
    i4_ret = x_sema_delete(pSystem->hEp0Lock);
    MUSB_ASSERT(OSR_OK == i4_ret);
                                              ...
                                              uint8_t bBsrPriority)
{
    ...
    if (pSystem)
    {
        ...
        pSystem->Services.pfEp0Lock = MGC_UcosEp0Lock;
        pSystem->Services.pfEp0Unlock = MGC_UcosEp0Unlock;
        ...
        VERIFY(OSR_OK == x_sema_create(&pSystem->ClassLock, X_SEMA_TYPE_BINARY,
                                       X_SEMA_STATE_UNLOCK));
        VERIFY(OSR_OK == x_sema_create(&pSystem->hEp0Lock, X_SEMA_TYPE_BINARY,
                                       X_SEMA_STATE_UNLOCK));
		...
		//klock
		snprintf(szPSbsr,sizeof(szPSbsr), "USB_%d", (int)pControllerInfo->dwInfo);
		
#else

        SPrintf(szPSbsr, "USB_%d", (int)pControllerInfo->dwInfo);
#endif
...
void MUSB_ResetPhy(uint32_t port)
{
	MUSB_UcosController *pControllerInfo = &(MUSB_aUcosController[port]);
	
	MUSB_BoardResetPhy(pControllerInfo->pBase);
}
#endif
