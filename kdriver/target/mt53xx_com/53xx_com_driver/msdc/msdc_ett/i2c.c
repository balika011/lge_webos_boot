...
S32 i2c_set_speed(mt_i2c *i2c)
{
  ...
  if((mode == i2c->mode) && (khz == i2c->speed)&&(i2c_timing_reg==I2C_TIMING_REG_BACKUP[i2c->id])) {
    ...
    //i2c->timing_reg = i2c_timing_reg;//copy to local
...
void _i2c_write_reg(mt_i2c *i2c)
{
  ...
  if(i2c->dma_en)
  {
    if (I2C_MASTER_RD == i2c->op) {
      __raw_writew(0x0000, i2c->pdmabase + OFFSET_INT_FLAG);
      __raw_writew(0x0001, i2c->pdmabase + OFFSET_CON);
      __raw_writew((U32)i2c->msg_buf, i2c->pdmabase + OFFSET_RX_MEM_ADDR);
      __raw_writew(i2c->trans_data.data_size, i2c->pdmabase + OFFSET_RX_LEN);
    } else if (I2C_MASTER_WR == i2c->op) {
      __raw_writew(0x0000, i2c->pdmabase + OFFSET_INT_FLAG);
      __raw_writew(0x0000, i2c->pdmabase + OFFSET_CON);
      __raw_writew((U32)i2c->msg_buf, i2c->pdmabase + OFFSET_TX_MEM_ADDR);
      __raw_writew(i2c->trans_data.data_size, i2c->pdmabase + OFFSET_TX_LEN);
    } else {
      __raw_writew(0x0000, i2c->pdmabase + OFFSET_INT_FLAG);
      __raw_writew(0x0000, i2c->pdmabase + OFFSET_CON);
      __raw_writew((U32)i2c->msg_buf, i2c->pdmabase + OFFSET_TX_MEM_ADDR);
      __raw_writew((U32)i2c->msg_buf, i2c->pdmabase + OFFSET_RX_MEM_ADDR);
      __raw_writew(i2c->trans_data.trans_len, i2c->pdmabase + OFFSET_TX_LEN);
      __raw_writew(i2c->trans_data.trans_auxlen, i2c->pdmabase + OFFSET_RX_LEN);
    }
    I2C_MB();
    __raw_writew(0x0001, i2c->pdmabase + OFFSET_EN);
 ...
 *   Returns: ERROR_CODE
 */
S32 mt_i2c_set_speed (U8 channel,U8 clock, I2C_SPEED_MODE mode, U8 khz)
{
  S32 ret = I2C_OK;
  I2CFUC();
  mt_i2c i2c;
...
//API to be remove
S32 mt_i2c_read(U8 channel,U8 addr,U8 *buffer, U32 len, U8 path_dir)
{
  return 0;
}

S32 mt_i2c_write (U8 channel,U8 addr,U8 *buffer, U32 len, U8 path_dir)
{
  return 0;
  }
S32 mt_i2c_read_new(struct mt_i2c_t *i2c,U8 *buffer, U32 len)
{
return 0;
}
S32 mt_i2c_write_new(struct mt_i2c_t *i2c,U8  *buffer, U32 len)
{
return 0;
  }
