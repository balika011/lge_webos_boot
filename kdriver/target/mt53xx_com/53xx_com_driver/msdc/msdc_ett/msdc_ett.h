...
#define MSDC_MT6595_WORKAROUND_FOR_CLK_SOURCE
#define MSDC_MT6595_WORKAROUND_FOR_ERR_HANDLE

#ifndef __KERNEL__
#define min(x, y) ((x < y) ? x : y)
#endif

#define ETT_PMIC_WRAP_READY

#define MSDC_MAX_NUM  (2)

/* window can set [1, 32] */
#define ETT_TUNE_WINDOW  (4)

/* dose msdc driver is ok? you can enable MSDC_BASIC_RW_TEST for test */
//#define MSDC_BASIC_RW_TEST 



//debug log 
#define ETT_STRIP_CODE  (1)
//#define ETT_MSDC_DBG_LOG
//#define ETT_MSDC_DBG_LOG_CMD
//#define ETT_MMC_DBG_LOG
//#define ETT_TEST_DBG_LOG
#define ETT_SHOW_PASS_WINDOW

/* designer suggest not to do spilt tune, all delay paras modify at the same time */
//#define ETT_SPILT_TUNE 

#define ETT_TEST_START_ADDR    (0x200000)  // offset is 1GB
#define ETT_TEST_MAZ_ADDR      (0x600000)  // max offset is 3GB
...
struct mmc_host
{
    ...
    u32 max_blk_size;
    u32 max_blk_count;
    u32 base_addr;    /* host base address */
...
struct mmc_host
{
    ...
    u32 timeout_ns;   /* data timeout ns */
    u32 timeout_clks; /* data timeout clks */
    u8  pll_src_clk;  /* system pll clock source */
...
#define MSDC_HS400			(1 << 11)  /* hs400 mode support(eMMC4.5)  */

#define MSDC_SMPL_RISING        (0)
#define MSDC_SMPL_FALLING       (1)
#define MSDC_SMPL_SEPERATE      (2)

/* which sytem PLL can be used for msdc_src_clk */
/* HF_FMSDC50_0 for mt6595 msdc pll source clock */
typedef enum
{
    HF_FMSDC50_0 = 0,
    
    MCPLL_CLK_DEFAULT = HF_FMSDC50_0,
    MCPLL_CLK_UNKOWN = 255,
} clk_source_t;
...
/* cause no mapping with each gears */
...
#define CTP_Wait_usec HAL_Delay_us
#define CTP_Wait_msec(ms) \
    do { \
...
extern void ett_memset(void* buf, int len, int val);

extern struct mmc_host ett_sd_host;
extern struct mmc_card ett_sd_card;

struct msdc_tune_para {
    unsigned char int_data_latch_cksel;       // clock timing relate
    unsigned char ckgen_msdc_dly_sel;     

    unsigned char cmd_int_data_latch_cksel; 
    unsigned char cmd_ckgen_msdc_dly_sel;     
    
    unsigned char cmd_rsp_ta_cntr;            // cmd pad relate
    unsigned char pad_cmd_rsp_rxdel;
    unsigned char iocon_rspl;
    unsigned char pad_tune_cmdrrdly;

#ifdef ETT_SPILT_TUNE
    unsigned char read_datx_sampl_edge[8];    // read pad relate
    unsigned char read_datx_delay[8];   
#else 
    unsigned char read_dat_sampl_edge;        // read pad relate
    unsigned char read_dat_delay;   
#endif

    unsigned char write_crcs_ta_cntr;         // write pad relate
    unsigned char write_pad_tune_datwrdly;   
    unsigned char write_w_d0_smpl;     
    unsigned char write_dat0_rddly;

    unsigned char hs400_ds_dly1;              //hs400 pad relate
    unsigned char hs400_ds_dly3;     
    
    unsigned char emmc50_cmdtxdly;            //tx pad relate
    unsigned char emmc50_dattxdly;     
    
    unsigned char cmd_max_window;
    unsigned char cmd_tx_max_window;
    unsigned char write_max_window;
    unsigned char write_tx_max_window;
    unsigned char hs400_max_window;
#ifdef ETT_SPILT_TUNE
    unsigned char read_max_window[8];
#else
    unsigned char read_max_window;
#endif
};

struct msdc_tune_debug {
    int cmd_count;
    int cmd_passed;
    int cmd_failed;
...
extern struct msdc_tune_debug g_tune_dbg;
extern struct msdc_tune_para g_tune_para;

extern u32 g_cmd_tune_done;
extern u32 g_msdc_ett_debug;
