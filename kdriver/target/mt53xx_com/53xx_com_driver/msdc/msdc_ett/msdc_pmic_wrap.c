...
/******************************************************************************
 * pmic_wrapper.c - MT6593 Linux pmic_wrapper Driver
 *
...
static S32 _pwrap_init_sistrobe( void )
{
  ...
  for( ind=0; ind<24; ind++)  // 24 sampling clock edge
  {
    ...
    WRAP_WR32(PMIC_WRAP_SIDLY ,0x3 - (ind & 0x3));
			...
			if( rdata == MT6331_DEFAULT_VALUE_READ_TEST ) {
				 ...
				 PWRAPLOG("_pwrap_init_sistrobe [Read Test of MT6331] fail,index=%d rdata=%x\n", ind,rdata);
			}
#endif
#ifdef SLV_6332
			...
			if( rdata == MT6332_DEFAULT_VALUE_READ_TEST ) {
				...
				result[1] |= (0x1 << ind);
				 ...
				 PWRAPLOG("_pwrap_init_sistrobe [Read Test of MT6332] fail,index=%d rdata=%x\n", ind,rdata);
			}
#endif  
	 }
#ifndef SLV_6331
  ...
  for( ind=23 ; ind>=0 ; ind-- )
  	{
  		for( i=0; i<2; i++){
   		 	if( ((result[i] & (0x1 << ind))!=0) && leading_one[i] == -1){
    			leading_one[i] = ind;
    		}
    	}
    	if((leading_one[0] & leading_one[1]) > 0) { break;}
  	}
  
  	for( ind=0 ; ind<24 ; ind++ )
  	{
  		for( i=0; i<2; i++){
    		if( ((result[i] & (0x1 << ind))!=0) && tailing_one[i] == -1){
    			tailing_one[i] = ind;
    		}
    	}
    	if((tailing_one[0] & tailing_one[1]) > 0) { break;}
  	}
...
void pwrap_dump_all_register(void)
{
  ...
  /*
  printf("dump peri_pwrap register\n");
