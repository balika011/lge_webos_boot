...
static INT32 _SifInternalMHLEdidReadSramCmd(INT32 i4Argc, const CHAR** szArgv);
...
static CLI_EXEC_T _arSifCmdTbl [] =
{
	...
	{ "redidmhlsram", NULL, _SifInternalMHLEdidReadSramCmd, NULL, "Sif write built-in HDMI 4K2K edid and set to internal SRAM", CLI_GUEST },
#endif
...
static INT32 _SifInternalMHLEdidReadSramCmd(INT32 i4Argc, const CHAR** szArgv)
{
	...
	for (i = 0; i < 1; i ++)
	{
		j = 0;
		Printf("MHL EDID%d:\n", i);
		while (j < FLASH_MHL_2_0_EDID_SIZE)
		{
			if ((j % 16) == 0)
			{
				Printf("0x%08X ", j);
			...
			if(IS_IC_5861() || (BSP_GetIcVersion() >= IC_VER_5890_AC))//only 5861.
			{
				vIO32Write1B(PDWNC_MHL_EDID_ADDR, j);
				Printf("0x%02X ", IO32ReadFldAlign(PDWNC_MHL_EDID_DATA, FLD_MHL_EDID_EDID_DATA));
			}
#endif
			if (((j + 1) % 16) == 0)
			{
